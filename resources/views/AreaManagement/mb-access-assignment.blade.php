                    ...
                    <div class="modal-body" style="overflow-y:auto; ">
                        ...
                        <form action="{{ route('INSERTROLEINDEXMB') }}" method="POST">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body">
                                    <span> Add Facility access to </span><span class="text-primary">{{ $SelectedMbName }}</span>
            ...
            @if(!$roleIndexData)
                <tr>
                    ...
                    <td class="text-center">
                        <center><input class="form-control" style="width: 16px; height: 16px;" type="checkbox" id="addaccesbox" value="" data-hcfid="{{ $facility['hcfcode'] }}"></center>
                    </td>
                </tr>
            @endif
        @endif
...
<div class="modal" id="remove-access">
            ...
            <div class="modal-body" style="overflow-y:auto;">
                <div class="col-md">
                    <form action="" method="POST">
                        ...
                        <div class="card shadow mb-4">
                            <div class="card-body">
                                <h6><span> Remove access from </span><span class="text-primary">{{ $SelectedMbName }}</span></h6>
                                <div class="table-responsive-sm" style="overflow-y:auto; max-height: 400px;margin-top:25px; margin-bottom: 10px; font-size: 10px;">
                                    <table class="table table-sm table-hover table-bordered table-light" id="tablemanager" width="100%" cellspacing="0">
                                        <thead>
                                            <tr>
    ...
    $roleIndexData = $RoleIndex->where('accessid', $facility['hcfcode'])->where('userid', $SelectedMbID)->first();
                        ...
                        <textarea id="accessid" required></textarea>
                    </form>
                </div>
            ...
            <div class="modal-body">
                <form action="{{ route('REMOVEROLEINDEXPRO') }}" method="POST">
                    ...
                    <div class="card bg-light text-dark" style="color:black; max-height: 250px; overflow-y:scroll; overflow-x:hidden;">
                        ...
                        @else                 
                            @foreach($Facilities as $facility)
                                <h6 id="confirmremovesubmission"><span class="col-md-8">{{ $facility['hcfname'] }}</span><span class="col-md-4 text-center controlnumber" style="float:right">{{ $facility['hcfcode'] }}</span></h6>
                            @endforeach
                        @endif
                        <br>
                    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        document.getElementById("openRemoveAccessModal").addEventListener("click", function() {
            ...
            var h6Elements = document.querySelectorAll('#confirmremovesubmission');
